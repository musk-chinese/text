...
struct OCP{
    ...
    byte mark2[2];//标志2
    byte len[2];//长度
    byte data[256];//修复：替换byte* data为固定数组
};
...
MAC bytesToMAC(byte* frame, byte Data[]){
    ...
    if (lenMAC < 80 || lenMAC > 1024) { // 限制最大长度，防止越界
    ...
    if (ocp_data_len > 0 && ocp_data_len <= 256) {
        ...
        memcpy(ocp.data, &frame[76], ocp_data_len); // 拷贝到ocp.data数组
    } else {
        memset(ocp.data, 0, sizeof(ocp.data));
        printf("警告：OCP数据长度异常 %d\n", ocp_data_len);
    }
...
//将MAC帧解封装，读取data数据（修复：printf冗余参数）
...
short getLenMAC(FILE *fp){
    ...
    // 修复：边界检查，防止负数/过小
    return (lenIP > 0) ? (lenIP + 18) : 0; // IP长度 + MAC头部(18) = MAC总长度
}   
...
int main(){
    ...
    while(true){
        ...
        // 修复：统一管理文件句柄，避免重复打开/关闭
        FILE *fp = fopen("test.txt","rb+");
        if (fp == NULL) {
            printf("文件打开失败！请先运行服务端写入数据\n");
            return 1;
        }

        // 获取MAC帧长度
        short lenMAC = getLenMAC(fp);
        if (lenMAC <= 0 || lenMAC > 1024) { // 限制最大长度，防止栈溢出
            printf("错误：MAC帧长度无效 %d\n", lenMAC);
            fclose(fp);
            continue;
        }
        ...
        //检验FCS校验和
        checkFCS(frame, lenMAC);
        //检验IP/UDP/OCP校验和
        checkSum(frame);

        // 解析MAC帧
        byte Data[lenMAC - 80];//OCP数据缓冲区
        MAC mac = bytesToMAC(frame, Data);
        //输出结构体内容
        displayMAC(mac);
        //解封装并校验
        decapsulateMAC(mac);
